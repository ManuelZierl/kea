    ...
    fn update_search_query(&mut self, cx: &mut Context<Self>) {
            ...
            .cloned()
        {
            self.editor.update(cx, |state, cx| {
                // Search result order does not describe viewport direction.
                // Allow either-axis reveal so an off-screen active match is
                // visible immediately after the query changes.
