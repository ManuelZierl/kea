    ...
    pub fn for_recommendation(
        ...
        let sections = match recommendation {
            Recommendation::SplitSeparator(range) => {
                ...
                if marker_line(text, cursor)?.1 != *range
                    || marker_line(text, cursor)?.0.trim() != "---"
                {
                    return None;
                }
            ...
            Recommendation::FillPlaceholder { name, .. } => {
                let value = value?;
                if !placeholders(text).iter().any(|(found, _)| found == name) {
                    return None;
                }
                let ranges = placeholder_ranges(text)
                    .into_iter()
                    .filter_map(|(found, range)| (found == name).then_some(range))
                    .collect::<Vec<_>>();
                let removed: usize = ranges.iter().map(|range| range.len()).sum();
                let size = text
                    .len()
                    .checked_sub(removed)?
                    .checked_add(ranges.len().checked_mul(value.len())?)?;
...
pub fn valid_prefix(prefix: &str) -> bool {
    prefix.is_empty()
        || (prefix.len() <= 16
            && prefix
                .bytes()
                .all(|b| b.is_ascii_punctuation() && !matches!(b, b'#' | b'=')))
}
...
pub fn recommend(text: &str, cursor: usize, prefix: &str) -> Vec<Recommendation> {
    ...
    if let Some((line, range)) = marker_line(text, cursor) {
        if line.trim() == "---" {
            result.push(Recommendation::SplitSeparator(range.clone()));
        }
        if !prefix.is_empty() && valid_prefix(prefix) {
            if let Some(query) = line.trim().strip_prefix(prefix) {
                if query.bytes().all(|b| b.is_ascii_alphabetic() || b == b'-') {
                    result.push(Recommendation::Actions {
                        query: query.into(),
                        range,
                    });
                }
    ...
    if unwrap_fence(text).is_some() {
        result.push(Recommendation::UnwrapFence);
    } else if separate_fences(text).is_some() {
        result.push(Recommendation::SeparateCodeBlocks);
    }
    result.extend(
        placeholders(text)
            .into_iter()
            .map(|(name, count)| Recommendation::FillPlaceholder { name, count }),
    );
    result
}
    ...
    for line in text.split_inclusive('\n') {
        ...
        if let Some((fence, start, body_start)) = open {
            if closes_fence(line, fence) {
                ...
                if result.len() > MAX_SECTIONS {
                    return None;
                }
                open = None;
            }
        } else if let Some(fence) = opening_fence(line) {
            open = Some((fence, offset, end));
        }
        offset = end;
    }
    if open.is_some() {
...
fn unwrap_fence(text: &str) -> Option<String> {
    ...
    let [(outer, body)] = ranges.as_slice() else {
        return None;
    };
    if !text[..outer.start].trim().is_empty() || !text[outer.end..].trim().is_empty() {
        return None;
    }
...
fn placeholder_ranges(text: &str) -> Vec<(&str, Range<usize>)> {
    ...
    while let Some(relative) = text[offset..].find("{{") {
        ...
        let body = start + 2;
        let Some(relative_end) = text[body..].find("}}") else {
            break;
        };
...
pub fn placeholders(text: &str) -> Vec<(String, usize)> {
    ...
    for (name, _) in placeholder_ranges(text) {
        if let Some((_, count)) = found.iter_mut().find(|(entry, _)| entry == name) {
            *count += 1;
        } else if found.len() < MAX_PLACEHOLDERS {
            found.push((name.into(), 1));
        }
    }
    found
}
