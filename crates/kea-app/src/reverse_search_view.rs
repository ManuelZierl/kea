...
use gpui::{
    anchored, deferred, div, prelude::*, px, App, ClipboardItem, Context, Corner, Div, Entity,
    EntityInputHandler, FocusHandle, FontWeight, KeyBinding, ScrollDelta, ScrollWheelEvent,
    Stateful, Subscription, Task, Timer, Window,
};
use gpui_component::{
...
impl ReverseSearchView {
    ...
    fn with_worker(
        ...
        let query = cx.new(|cx| {
            InputState::new(window, cx).placeholder("Search input history and saved memories")
        });
    ...
    fn item_action(&mut self, action: ItemAction, window: &mut Window, cx: &mut Context<Self>) {
        if self.operation_pending || self.composing(window, cx) {
            return;
        }
        let Some(hit) = self.ready_hit().cloned() else {
            return;
    ...
    fn accept(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        match self.mode.clone() {
            ...
            Mode::ConfirmDelete(ids) => {
                self.operation_pending = self.request(Request::Delete(ids), cx);
            }
        }
    }
    ...
    fn overlay_action(
        ...
        match action.command {
            ...
            Command::Actions => {
                if matches!(self.mode, Mode::Naming(_) | Mode::ConfirmDelete(_)) {
...
impl Render for ReverseSearchView {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        match self.mode.clone() {
            Mode::Results => {
                ...
                if self.hits.is_empty() {
                    content = content.child(div().p_3().child(if self.worker.is_none() {
                        ...
                        "No matches. Try another query, or save the original draft."
                    }));
                }
                ...
                for (index, hit) in self.hits.iter().enumerate().skip(start).take(PAGE_SIZE) {
                    ...
                    let label = if hit.entry.name.is_some() {
                        ...
                        format!(
                            "{} · {} retained occurrence(s) · {}",
                            hit.entry.kind.label(),
                            hit.occurrence_ids.len(),
                            if hit.local {
                                "this directory"
                            } else {
                    ...
                    content = content.child(
                        div()
                            ...
                            .on_hover(cx.listener(move |this, hovered, _, cx| {
                                if *hovered
                                    ...
                                    && this
                                        .hits
                                        .get(index)
                                        .is_some_and(|hit| hit.entry.id == hover_id)
                                {
                                    this.selected = index;
                                    cx.notify();
                                }
                            ...
                            .on_click(cx.listener(move |this, _, window, cx| {
                                if this.visible_generation == Some(generation)
                                    && this.generation == generation
                                    && this.hits.get(index).is_some_and(|hit| hit.entry.id == id)
                                {
                                    this.selected = index;
                ...
                content = content.child(div().text_sm().child(format!(
                    ...
                    if self.hits.len() == MAX_RESULTS {
                        " (top matches; refine to find more)"
                    } else {
                        ""
                    },
                )));
            ...
            Mode::Actions => {
                ...
                content = content.child(
                    div()
                        .text_sm()
                        .child("↑↓ choose action · Enter select · Esc back"),
                );
            }
            Mode::Naming(entry) => {
                content = content
                    ...
                    .child(
                        div()
                            .text_sm()
                            .child("Saved text is unencrypted local data. No command will run."),
                    )
        ...
        if self.operation_pending {
            content = content.child(div().text_sm().child("Saving changes…"));
        }
...
mod tests {
    ...
    fn search_cancellation_preserves_editor_entity_text_and_selection(cx: &mut TestAppContext) {
        ...
        window
            .update(cx, |_, window, cx| {
                let draft = command_editor::new_draft(
                    None,
                    &Settings::default(),
                    "a😀\noriginal",
                    window,
                ...
                let before =
                    draft.update(cx, |state, cx| state.selected_text_range(true, window, cx));
    ...
    fn insertion_uses_utf16_editor_replacement_and_refuses_stale_or_composing_drafts(
        ...
        window
            .update(cx, |_, window, cx| {
                let draft =
                    command_editor::new_draft(None, &Settings::default(), "a😀b", window, cx);
                let search = cx.new(|cx| {
                    ReverseSearchView::with_worker(Worker::start(None, false), false, window, cx)
                });
                search.update(cx, |search, cx| {
