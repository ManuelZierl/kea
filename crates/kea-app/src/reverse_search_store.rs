...
pub fn data_directory() -> Option<PathBuf> {
    if let Some(path) = std::env::var_os("KEA_MEMORY_DIR") { return Some(path.into()); }
    if cfg!(target_os = "windows") {
        return std::env::var_os("LOCALAPPDATA").map(PathBuf::from).map(|p| p.join("Kea/input-memory"));
    }
    ...
    std::env::var_os("XDG_DATA_HOME").map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(PathBuf::from).map(|p| p.join(".local/share")))
...
impl Store {
    ...
    fn check_directory(&self, create: bool) -> io::Result<()> {
        if !self.directory.is_absolute() { return Err(invalid("KEA_MEMORY_DIR must be absolute")); }
        ...
        if !metadata.is_dir() || metadata.file_type().is_symlink() {
            return Err(invalid("Input-memory directory must be a real directory, not a symlink"));
        }
        #[cfg(unix)]
        {
            use std::os::unix::fs::PermissionsExt as _;
            if metadata.permissions().mode() & 0o077 != 0 {
                return Err(invalid("Input-memory directory is not private; restrict its permissions to 0700"));
            }
        }
        Ok(())
    }
    ...
    pub fn load(&self) -> Result<(Library, Vec<String>), String> {
        match self.check_directory(false) {
            Err(error) if error.kind() == io::ErrorKind::NotFound => return Ok((Library::default(), vec![])),
            result => result.map_err(|e| e.to_string())?,
        }
        let mut library = Library::default();
        let mut warnings = Vec::new();
        let mut paths = Vec::new();
        for (count, file) in fs::read_dir(&self.directory).map_err(|e| e.to_string())?.enumerate() {
            if count >= MAX_ENTRIES * 2 { return Err("Too many files in the input-memory directory; loading stopped.".into()); }
            let file = file.map_err(|e| e.to_string())?;
            let path = file.path();
            if path.extension().and_then(|s| s.to_str()) != Some("kmem") { continue; }
            if paths.len() >= MAX_ENTRIES { return Err("Stored input memory exceeds the 5,000-entry limit.".into()); }
            paths.push(path);
        }
        // Deterministic partial recovery if individual files are damaged.
        paths.sort();
        for path in paths {
            let result = read_entry(&path).map_err(|e| e.to_string()).and_then(|entry| library.insert(entry));
            if let Err(error) = result {
                if warnings.len() < 3 { warnings.push(format!("Skipped an input-memory entry: {error}")); }
            }
        }
        Ok((library, warnings))
    }
    ...
    pub fn remove(&self, id: &str) -> Result<(), String> {
        if id.is_empty() || id.len() > 100 || !id.bytes().all(|b| b.is_ascii_hexdigit() || b == b'-') {
...
fn encode(entry: &Entry) -> Vec<u8> {
    ...
    out.push(match entry.kind { InputKind::Posix => 0, InputKind::PowerShell => 1, InputKind::Application => 2 });
    ...
    for value in [&entry.directory, &entry.name, &entry.scope] {
        ...
        if let Some(value) = value { text(&mut out, value); }
    }
    out
}
...
fn read_entry(path: &Path) -> io::Result<Entry> {
    ...
    if !metadata.is_file() || metadata.file_type().is_symlink() || metadata.len() > MAX_FILE_BYTES as u64 {
        return Err(invalid("Not a bounded regular memory file"));
    }
    let mut bytes = Vec::new();
    File::open(path)?.take((MAX_FILE_BYTES + 1) as u64).read_to_end(&mut bytes)?;
    if bytes.len() > MAX_FILE_BYTES { return Err(invalid("Memory file grew beyond its limit")); }
...
fn decode(bytes: &[u8]) -> io::Result<Entry> {
    fn take<'a>(bytes: &mut &'a [u8], count: usize) -> io::Result<&'a [u8]> {
        let result = bytes.get(..count).ok_or_else(|| invalid("Truncated memory file"))?;
        *bytes = &bytes[count..];
        Ok(result)
    }
    fn text(bytes: &mut &[u8]) -> io::Result<String> {
        let length = u32::from_le_bytes(take(bytes, 4)?.try_into().unwrap()) as usize;
        let text = std::str::from_utf8(take(bytes, length)?).map_err(|_| invalid("Invalid UTF-8 in memory"))?;
        Ok(text.to_string())
    }
    fn optional(bytes: &mut &[u8]) -> io::Result<Option<String>> {
        match take(bytes, 1)?[0] { 0 => Ok(None), 1 => text(bytes).map(Some), _ => Err(invalid("Invalid optional field")) }
    }
    let mut bytes = bytes;
    if take(&mut bytes, MAGIC.len())? != MAGIC { return Err(invalid("Unsupported memory format/version")); }
    let kind = match take(&mut bytes, 1)?[0] {
        0 => InputKind::Posix, 1 => InputKind::PowerShell, 2 => InputKind::Application,
        _ => return Err(invalid("Unknown input kind")),
    };
    let submitted_ms = u64::from_le_bytes(take(&mut bytes, 8)?.try_into().unwrap());
    let entry = Entry {
        id: text(&mut bytes)?, text: text(&mut bytes)?, kind, submitted_ms,
        directory: optional(&mut bytes)?, name: optional(&mut bytes)?, scope: optional(&mut bytes)?,
    };
    if !bytes.is_empty() { return Err(invalid("Unexpected trailing memory data")); }
...
mod tests {
    ...
    impl Temporary {
        fn new() -> Self {
            Self(std::env::temp_dir().join(format!("kea-memory-{}", Entry::submitted("test".into(), InputKind::Posix, None).id)))
        }
    }
    ...
    fn round_trip_keeps_unicode_multiline_empty_options_and_scopes() {
        ...
        let mut entry = Entry::submitted("printf '😀\\n'\necho ä".into(), InputKind::Posix, Some("/tmp/a b".into()));
    ...
    fn truncated_and_unknown_versions_fail_without_accepting_partial_records() {
        ...
        for length in 0..encoded.len() { assert!(decode(&encoded[..length]).is_err()); }
    ...
    fn independent_instances_do_not_overwrite_each_others_entries() {
        ...
        a.write(&Entry::submitted("one".into(), InputKind::Posix, None)).unwrap();
        b.write(&Entry::submitted("two".into(), InputKind::Posix, None)).unwrap();
        assert_eq!(a.load().unwrap().0.entries().len(), 2);
    }
    #[test]
    fn opening_missing_storage_does_not_create_or_write_anything() {
        let root = Temporary::new();
        assert!(Store::new(root.0.clone()).load().unwrap().0.entries().is_empty());
    ...
    fn files_are_private_and_symlink_directories_are_rejected() {
        ...
        store.write(&entry).unwrap();
        assert_eq!(fs::metadata(&root.0).unwrap().permissions().mode() & 0o777, 0o700);
        assert_eq!(fs::metadata(root.0.join(format!("{}.kmem", entry.id))).unwrap().permissions().mode() & 0o777, 0o600);
