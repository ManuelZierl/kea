...
impl Render for DraggedTab {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        div().px_3().py_1().bg(cx.theme().background).child(self.label.clone())
    }
}
...
impl KeaRoot {
    pub(super) fn new(view: Entity<KeaView>, window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let mut root = Self {
            tabs: Tabs::default(), keymap, settings, focus: cx.focus_handle(),
            scroll: ScrollHandle::new(), notice: None, close_allowed: false,
        };
        let title = if view.read(cx).session.is_running() { "Terminal" } else { "Recording" };
        root.attach(view, title.into(), initial_focus, window, cx);
        root
    }

    fn attach(
        &mut self, view: Entity<KeaView>, title: String, last_focus: InitialFocus,
        ...
        let events = cx.subscribe_in(&view, window, move |this, _, event, window, cx| {
            match event {
                ...
                WorkspaceEvent::SettingsChanged(settings, change) => {
                    ...
                    let others: Vec<_> = this.tabs.iter().filter(|(other, _)| *other != id)
                        .map(|(_, tab)| tab.view.clone()).collect();
                    for other in others {
        ...
        let tab = TerminalTab { view, title, last_focus, _events: events, _changes: changes };
        assert!(self.tabs.insert(tab).is_ok(), "terminal capacity checked before spawning");
        self.reveal_active();
    }

    fn reveal_active(&self) {
        if let Some(index) = self.tabs.iter().position(|(id, _)| Some(id) == self.tabs.active_id()) {
    ...
    fn activate(&mut self, id: TabId, window: &mut Window, cx: &mut Context<Self>) {
        if window.has_active_dialog(cx) || self.tabs.get(id).is_none() { return; }
    ...
    fn new_terminal(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if self.tabs.is_full() {
            self.notice = Some(format!("At most {MAX_TERMINALS} terminals can be open. Close one before opening another."));
            cx.notify();
            return;
        }
        ...
        let focus = if shell.is_some() { InitialFocus::Editor } else { InitialFocus::Terminal };
        let view = cx.new(|cx| KeaView::new(
            session, document, shell, self.keymap.clone(), self.settings.clone(),
            focus, None, window, cx,
        ));
    ...
    fn needs_confirmation(tab: &TerminalTab, cx: &App) -> bool {
        ...
        view.session.is_running() || !view.editor.read(cx).value().is_empty()
    ...
    fn remove_tab(&mut self, id: TabId, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if self.tabs.remove(id).is_some() {
            ...
            if selected { self.focus_selected(window, cx); }
            cx.notify();
        }
    }

    pub(super) fn should_close(&mut self, window: &mut Window, cx: &mut Context<Self>) -> bool {
        if self.close_allowed || !self.tabs.iter().any(|(_, tab)| Self::needs_confirmation(tab, cx)) {
            return true;
        }
    ...
    fn request_close_window(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        if window.has_active_dialog(cx) { return; }
        if !self.tabs.iter().any(|(_, tab)| Self::needs_confirmation(tab, cx)) {
    ...
    fn invoke_action(&mut self, action: Action, window: &mut Window, cx: &mut Context<Self>) {
        ...
        match action {
            ...
            Action::CloseTerminal => {
                if let Some(id) = self.tabs.active_id() { self.close_tab(id, window, cx); }
            }
...
impl Render for KeaRoot {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let mut strip = div().id("terminal-tabs").flex().items_center().h(px(34.))
            .min_w_0().flex_1().overflow_x_scroll().track_scroll(&self.scroll);
        for (id, tab) in self.tabs.iter() {
            ...
            let suffix = if !view.session.is_running() { " · ended" }
                else if !view.session.input_allowed() { " · history" } else { "" };
            let label = if !context.is_empty() && context != "local" {
                format!("{} · {context}{suffix}", tab.title)
            } else { format!("{} {}{suffix}", tab.title, id.0 + 1) };
            let drag = DraggedTab { id, label: label.clone() };
            strip = strip.child(
                div().id(("terminal-tab", id.0)).flex().items_center().flex_shrink_0()
                    .h_full().px_1().border_b_2()
                    .border_color(if active == Some(id) { cx.theme().primary } else { cx.theme().border })
                    ...
                    .child(Button::new(("activate-terminal", id.0)).label(label)
                        .ghost().small().selected(active == Some(id))
                        .on_click(cx.listener(move |this, _, window, cx| this.activate(id, window, cx))))
                    .child(Button::new(("close-terminal", id.0)).icon(IconName::X)
                        .tooltip("Close terminal").ghost().small()
                        .on_click(cx.listener(move |this, _, window, cx| this.close_tab(id, window, cx)))),
            );
        }
        ...
        let body = if let Some(tab) = self.tabs.active() {
            ...
            div().flex_1().flex().items_center().justify_center()
                .child("No terminals open. Use + or the New terminal shortcut to open a local shell.")
        };
        div().size_full().flex().flex_col().key_context("KeaChrome").track_focus(&self.focus)
            .on_action(cx.listener(|this, action: &Invoke, window, cx| this.invoke_action(action.action, window, cx)))
            .bg(cx.theme().background).text_color(cx.theme().foreground)
            .child(TitleBar::new().child(div().font_weight(FontWeight::BOLD).child("Kea"))
                .on_close_window(cx.listener(|this, _, window, cx| this.request_close_window(window, cx))))
            .child(div().flex().items_center().flex_shrink_0().border_b_1().border_color(cx.theme().border)
                .child(strip)
                .child(Button::new("new-terminal").icon(IconName::Plus).ghost().small()
                    .tooltip(format!("New local terminal · {}", self.keymap.label(Action::NewTerminal)))
                    .disabled(self.tabs.is_full())
                    .on_click(cx.listener(|this, _, window, cx| this.new_terminal(window, cx)))))
            .children(self.notice.clone().map(|notice| div().px_2().py_1().text_color(cx.theme().danger).child(notice)))
            .child(body).children(dialog_layer)
    }
}
