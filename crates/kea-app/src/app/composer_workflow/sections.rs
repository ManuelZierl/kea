...
impl KeaView {
    ...
    pub(in crate::app) fn validate_workflow(
        &mut self,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        ...
        let valid = self.composer_workflow.menu.as_ref().is_none_or(|menu| {
            menu.target == self.editor
                ...
                && (menu.focus.is_focused(window)
                    || menu.value.focus_handle(cx).is_focused(window)
                    || menu
                        .preview_editors
                        .iter()
                        .any(|e| e.focus_handle(cx).contains_focused(window, cx)))
        });
        if !valid {
            self.composer_workflow.menu = None;
        }
    ...
    pub(in crate::app) fn has_pending_sections(&self, cx: &App) -> bool {
        self.composer_workflow
            .sections
            .iter()
            .any(|editor| !editor.read(cx).value().is_empty())
    }
    ...
    pub(in crate::app) fn select_composer_section(&mut self, index: usize, cx: &mut Context<Self>) {
        let Some(editor) = self.composer_workflow.sections.get(index).cloned() else {
            return;
        };
        self.cancel_workflow();
    ...
    pub(in crate::app) fn navigate_composer(
        &mut self,
        forward: bool,
        window: &mut Window,
        cx: &mut Context<Self>,
    ...
    fn install_layout(
        ...
        self.composer_workflow.subscriptions =
            observe_sections(&self.composer_workflow.sections, cx);
    ...
    fn retain_layout_change(&mut self, before: LayoutSnapshot, cx: &App) {
        ...
        while workflow.undo.len() > MAX_LAYOUT_HISTORY
            || workflow.undo.iter().map(LayoutChange::bytes).sum::<usize>()
                > MAX_LAYOUT_HISTORY_BYTES
        {
    ...
    pub(in crate::app) fn split_composer(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if let Some(plan) = EditPlan::split(&text, cursor) {
            self.apply_composer_plan(plan, window, cx);
        } else {
            self.notice = Some(
                "This draft exceeds the 256 KiB editor-action limit; it was not changed.".into(),
            );
            cx.notify();
        }
    }

    pub(in crate::app) fn apply_composer_plan(
        &mut self,
        plan: EditPlan,
        window: &mut Window,
        cx: &mut Context<Self>,
        ...
        {
            self.notice =
                Some("The draft changed; open composer actions again. Nothing was applied.".into());
            cx.notify();
            return;
        }
        ...
        if plan.sections.is_empty() || new_count > workflow::MAX_SECTIONS {
            self.notice =
                Some("At most 32 composer sections are allowed. Nothing was changed.".into());
            cx.notify();
            return;
        }
        self.pending_run = None;
        ...
        if plan.sections.len() == 1 {
            self.editor.update(cx, |state, cx| {
                state.replace_text_in_range(
                    Some(0..plan.source.encode_utf16().count()),
                    &plan.sections[0],
                    window,
                    cx,
                );
            ...
            if before.bytes().saturating_mul(2)
                + plan.sections.iter().map(String::len).sum::<usize>()
                > MAX_LAYOUT_HISTORY_BYTES
            {
                self.notice = Some(
                    "The structural-undo budget is full for this draft. Nothing was changed."
                        .into(),
                );
                cx.notify();
                return;
            ...
            let replacements = plan
                .sections
                .iter()
                .map(|text| command_editor::new_draft(self.shell, &self.settings, text, window, cx))
                .collect::<Vec<_>>();
    ...
    pub(in crate::app) fn merge_composer(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        if !self.editor_available(window, cx) || self.composer_workflow.active == 0 {
            return;
        }
        let before = self.snapshot_layout(cx);
        let index = self.composer_workflow.active;
        let merged = format!("{}\n{}", before.values[index - 1], before.values[index]);
        if merged.len() > workflow::MAX_ACTION_BYTES
            || before.bytes().saturating_mul(2) + merged.len() > MAX_LAYOUT_HISTORY_BYTES
        {
            self.notice =
                Some("Merged text exceeds the editor-action budget. Nothing was changed.".into());
            cx.notify();
            return;
        }
        command_editor::cancel_submission_candidate(cx);
        ...
        let replacement =
            command_editor::new_draft(self.shell, &self.settings, &merged, window, cx);
        editors.splice(index - 1..index + 1, [replacement]);
        self.install_layout(editors, index - 1, window, cx);
        self.retain_layout_change(before, cx);
    }

    pub(in crate::app) fn undo_composer_layout(
        &mut self,
        redo: bool,
        window: &mut Window,
        cx: &mut Context<Self>,
        ...
        if redo {
            self.composer_workflow.undo.push(change);
        } else {
            self.composer_workflow.redo.push(change);
        }
    }
    ...
    pub(in crate::app) fn finish_section_submission(
        &mut self,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        ...
        if index == editors.len() {
            editors.push(command_editor::new_draft(
                self.shell,
                &self.settings,
                "",
                window,
    ...
    pub(in crate::app) fn send_selection(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if let Some(text) = selected {
            self.request_submission(text, false, window, cx);
        }
    }
}
