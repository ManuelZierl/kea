...
impl KeaView {
    pub(in crate::app) fn workflow_keystroke(
        &mut self,
        ...
        if self
            ...
            .owned_keys
            .iter()
            .any(|key| key == physical)
        {
            cx.stop_propagation();
            return true;
        }
        if self.interrupt_keystroke(event, window, cx) {
            return true;
        }
        ...
        let submission = matches!(
            action,
            Some(Action::RunShell | Action::SendApplication | Action::SendSelection)
        );
        ...
        if self.composer_workflow.menu.is_some() {
            ...
            if let Some(menu) = &mut self.composer_workflow.menu {
                ...
                if key.key == "escape" {
                    ...
                    if plain && fresh {
                        if menu.preview.is_some() {
                            self.accept_composer_preview(window, cx);
                        } else if menu.placeholder.is_some() {
                            self.preview_placeholder(window, cx);
                        } else {
                            let index = menu.selected;
                            self.preview_composer_action(index, window, cx);
                        }
                    }
                } else if menu.focus.is_focused(window) && matches!(key.key.as_str(), "up" | "down")
                {
                    if !menu.items.is_empty() {
                        menu.selected = if key.key == "down" {
                            (menu.selected + 1) % menu.items.len()
                        } else {
                            (menu.selected + menu.items.len() - 1) % menu.items.len()
                        };
                    }
                } else if submission {
        ...
        if self.editor.focus_handle(cx).is_focused(window)
            && (submission || self.pending_run.is_some() && enter)
        {
            self.composer_workflow.owned_keys.push(physical.to_owned());
            if !fresh {
                cx.stop_propagation();
