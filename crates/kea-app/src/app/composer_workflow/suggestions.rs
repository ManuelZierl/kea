...
impl KeaView {
    pub(in crate::app) fn composer_recommendations(&mut self, cx: &App) -> Vec<Recommendation> {
        ...
        if let Some((editor, old, old_cursor, old_prefix, recommendations)) =
            &self.composer_workflow.recommendations
        {
        ...
        self.composer_workflow.recommendations =
            (text.len() <= workflow::MAX_ACTION_BYTES).then(|| {
                (
                    self.editor.clone(),
                    text,
                    cursor,
    ...
    pub(in crate::app) fn open_composer_actions(
        &mut self,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        ...
        if source.len() > workflow::MAX_ACTION_BYTES {
            self.notice =
                Some("Composer actions analyze at most 256 KiB. The draft is unchanged.".into());
            cx.notify();
            return;
        }
        ...
        let mut items = recommendations
            .into_iter()
            .filter(|rec| !matches!(rec, Recommendation::Actions { .. }))
            .map(ComposerAction::Recommendation)
            .collect::<Vec<_>>();
        items.extend([
            ComposerAction::Split,
            ComposerAction::Add,
            ComposerAction::Merge,
            ComposerAction::Previous,
            ComposerAction::Next,
            ComposerAction::UndoLayout,
            ComposerAction::RedoLayout,
            ComposerAction::History,
            ComposerAction::SendSelection,
        ]);
        if let Some((query, _)) = &trigger {
            let query = if query == "memory" {
                "saved memory"
            } else {
                query.as_str()
            };
            ...
            items.retain(|item| {
                matches!(
                    item,
                    ComposerAction::Split | ComposerAction::Add | ComposerAction::History
        ...
        window.focus(&focus);
        let value =
            cx.new(|cx| InputState::new(window, cx).placeholder("Literal replacement value"));
        self.composer_workflow.menu = Some(ComposerMenu {
            target: self.editor.clone(),
            source,
            cursor,
            items,
            selected: 0,
            prefix_range: trigger.map(|(_, range)| range),
            placeholder: None,
            value,
            preview: None,
            preview_editors: Vec::new(),
            focus,
        });
        cx.notify();
    }

    pub(in crate::app) fn dismiss_composer_actions(
        &mut self,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::app) fn preview_composer_action(
        &mut self,
        index: usize,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        let Some(menu) = &self.composer_workflow.menu else {
            return;
        };
        let Some(action) = menu.items.get(index).cloned() else {
            return;
        };
        let source = menu.source.clone();
        let cursor = menu.cursor;
        let prefix_range = menu.prefix_range.clone();
        if menu.target != self.editor
            || self.editor.read(cx).value().as_ref() != source
            || self.editor.read(cx).cursor() != cursor
        {
            return;
        }
        match action {
            ...
            action => {
                ...
                if matches!(action, ComposerAction::History) {
                    if let Some(range) = prefix_range {
                        let utf16 = source[..range.start].encode_utf16().count()
                            ..source[..range.end].encode_utf16().count();
                        self.editor.update(cx, |state, cx| {
                            state.replace_text_in_range(Some(utf16), "", window, cx)
                        });
                    }
                }
                match action {
    ...
    pub(in crate::app) fn preview_placeholder(
        &mut self,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        ...
        if menu.value.update(cx, |state, cx| {
            state.marked_text_range(window, cx).is_some()
        }) {
            return;
        }
        let plan = EditPlan::for_recommendation(
            &menu.source,
            menu.cursor,
            rec,
            Some(&menu.value.read(cx).value()),
        );
        if let Some(plan) = plan {
            self.show_composer_preview(plan, window, cx);
        } else {
            self.notice =
                Some("Replacement exceeds the 256 KiB action limit. Nothing was changed.".into());
            cx.notify();
        }
    }

    pub(in crate::app) fn show_composer_preview(
        &mut self,
        plan: EditPlan,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        let Some(menu) = &mut self.composer_workflow.menu else {
            return;
        };
        menu.preview_editors = plan
            .sections
            .iter()
            .map(|text| {
                cx.new(|cx| {
    ...
    pub(in crate::app) fn accept_composer_preview(
        &mut self,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        let Some(menu) = self.composer_workflow.menu.take() else {
            return;
        };
