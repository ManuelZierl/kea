...
impl KeaView {
    pub(super) fn complete(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if self.completion_busy || self.input_mode != InputMode::Document {
            return;
        }
        ...
        let (cursor, selected) = self.editor.update(cx, |state, cx| {
            (
                state.cursor(),
                state
        ...
        let request = Request {
            text: text.clone(),
            cursor,
            directory: directory.as_ref().map(Into::into),
            shell,
            history: self
                .document
                .blocks()
        ...
        let job = cx
            .background_executor()
            .spawn(async move { completion::suggest(request) });
    ...
    pub(super) fn accept_completion(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        self.editor.update(cx, |state, cx| {
            state.replace_text_in_range(Some(start..end), &item.replacement, window, cx)
        });
        self.notice = None;
        cx.notify();
    }
        ...
        let mut choices = div()
            ...
            .child(
                div()
                    ...
                    .child("Completions · Tab/Enter accept · ↑/↓ choose · Esc dismiss"),
            );
