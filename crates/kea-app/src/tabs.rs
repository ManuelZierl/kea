...
impl<T> Tabs<T> {
    ...
    pub fn is_full(&self) -> bool {
        self.len() >= MAX_TERMINALS
    }
    pub fn next_id(&self) -> TabId {
        TabId(self.next_id)
    }
    ...
    pub fn insert(&mut self, value: T) -> Result<TabId, T> {
        if self.is_full() || self.next_id == u64::MAX {
            return Err(value);
        }
        let id = self.next_id();
        self.next_id += 1;
    ...
    pub fn adjacent(&self, forward: bool) -> Option<TabId> {
        let at = self
            .entries
            .iter()
            .position(|entry| Some(entry.0) == self.active)?;
        let next = if forward {
            (at + 1) % self.len()
        } else {
            (at + self.len() - 1) % self.len()
        };
    ...
    pub fn remove(&mut self, id: TabId) -> Option<T> {
        ...
        if self.active == Some(id) {
            self.active = self
                .entries
                .get(at.min(self.len().saturating_sub(1)))
                .map(|entry| entry.0);
        }
        Some(value)
    }
    ...
    pub fn reorder(&mut self, id: TabId, target: TabId) -> bool {
        let Some(from) = self.entries.iter().position(|entry| entry.0 == id) else {
            return false;
        };
        let Some(to) = self.entries.iter().position(|entry| entry.0 == target) else {
            return false;
        };
        if from == to {
            return false;
    ...
    pub fn move_active(&mut self, forward: bool) -> bool {
        ...
        let Some(at) = self.entries.iter().position(|entry| entry.0 == id) else {
            return false;
        };
