...
fn token(text: &str, cursor: usize, shell: ShellFlavor) -> Option<Token> {
    ...
    while let Some((offset, c)) = iter.next() {
        ...
        if let Some(q) = quote {
            if c == q {
                if shell == ShellFlavor::PowerShell
                    && q == '\''
                    && iter.peek().is_some_and(|(_, n)| *n == '\'')
                {
                    iter.next();
                    current.push('\'');
                } else {
    ...
    let directories_only = words.first().is_some_and(|word| {
        matches!(
            word.to_ascii_lowercase().as_str(),
            "cd" | "chdir" | "set-location" | "pushd" | "push-location"
        )
    });
    Some(Token {
        range: start..end,
    ...
    if !value.is_empty()
        && value.chars().all(|c| {
            c.is_ascii_alphanumeric()
                || matches!(c, '/' | '.' | '_' | '-' | ':')
                || (c == '\\' && shell == ShellFlavor::PowerShell)
        })
    {
        return value.into();
    }
...
pub fn suggest(request: Request) -> Vec<Suggestion> {
    ...
    let prefix = match request.text.get(..request.cursor) {
        Some(value) => value,
        None => return vec![],
    };
        ...
        for command in request.history.iter().take(64) {
            if command != prefix && command.starts_with(prefix) && seen.insert(command.clone()) {
                suggestions.push(Suggestion {
                    label: format!("History · {}", command.replace('\n', " ↵ ")),
                    replacement: command.clone(),
                    range: 0..request.cursor,
                });
            }
        }
    }
    let Some(token) = token(&request.text, request.cursor, request.shell) else {
        return suggestions;
    };
    let Some(cwd) = request.directory.filter(|p| p.is_absolute()) else {
        return suggestions;
    };
    let split = token
        .value
        .rfind(|c| c == '/' || (c == '\\' && request.shell == ShellFlavor::PowerShell))
        .map_or(0, |i| i + 1);
    ...
    if directory_prefix.starts_with('~') {
        return suggestions;
    }
    ...
    if token.command && directory_prefix.is_empty() {
        candidates.extend(request.path.into_iter().take(128).map(|path| {
            (
                if path.is_absolute() {
                    path
                } else {
    ...
    for (directory, from_path) in candidates {
        if budget == 0 || Instant::now() >= deadline || suggestions.len() >= LIMIT {
            break;
        }
        ...
        for entry in entries.flatten() {
            ...
            if name.len() > 4096 || !matches_prefix(name, name_prefix, request.shell) {
                continue;
            }
            let is_dir = entry.path().is_dir();
            if token.directories_only && !is_dir {
                continue;
            }
            if from_path && !executable(&entry.path()) {
                continue;
            }
            let separator = if request.shell == ShellFlavor::PowerShell {
                "\\"
            } else {
                "/"
            };
            let mut value = if from_path {
                name.to_string()
            } else {
...
mod tests {
    ...
    fn parses_windows_paths_and_partial_quotes_without_evaluation() {
        ...
        assert_eq!(
            quote_literal("a'b $c", ShellFlavor::PowerShell, false),
            "'a''b $c'"
        );
        assert_eq!(
            quote_literal("my app.exe", ShellFlavor::PowerShell, true),
            "& 'my app.exe'"
        );
    ...
    fn ranges_preserve_unicode_prefixes_and_following_arguments() {
        ...
        completed.replace_range(
            t.range,
            &quote_literal("my file", ShellFlavor::Posix, false),
        );
    ...
    fn completes_files_and_directories_relative_to_reported_cwd() {
        ...
        let request = |text: &str| Request {
            text: text.into(),
            cursor: text.len(),
            directory: Some(base.clone()),
            shell: ShellFlavor::Posix,
            history: vec![],
            path: vec![],
        };
