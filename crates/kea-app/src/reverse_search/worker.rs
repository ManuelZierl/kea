...
impl State {
    fn refresh(&mut self) -> Result<Vec<String>, String> {
        ...
        for entry in self
            ...
            .entries()
            .iter()
            .filter(|entry| self.ephemeral.contains(&entry.id))
        {
    ...
    fn handle(&mut self, request: Request) -> Option<Reply> {
        match request {
            ...
            Request::Search {
                ...
                let (hits, error) =
                    match self.library.search(&query, directory.as_deref(), now_ms()) {
                        Ok(hits) => (hits, None),
            ...
            Request::Record(entry) => {
                ...
                if entry.text.starts_with(' ') || entry.text.trim().is_empty() {
                    return None;
                }
                if let Err(error) = self.library.insert(entry.clone()) {
                    return Some(Reply::Warning(error));
                }
                ...
                if self.persist_history {
                    match self
                        .store
                        .as_ref()
                        .ok_or_else(|| "No input-memory data directory is available.".to_string())
            ...
            Request::Save(entry) => {
                if entry.name.is_none() {
                    return Some(Reply::OperationFailed(
                        "Saved memories require a name.".into(),
                    ));
                }
                if self.library.entries().iter().any(|old| {
                    old.id != entry.id
                        && old.name == entry.name
            ...
            Request::Delete(ids) => {
                let total = ids.len();
                for (forgotten, id) in ids.into_iter().enumerate() {
                    ...
                    if !self.ephemeral.contains(&id) {
                        if let Some(store) = &self.store {
                            if let Err(error) = store.remove(&id) {
                                return Some(Reply::DeleteFailed(format!(
                                    "Forgot {forgotten} of {total} records. {error}"
                                )));
                            }
                        }
                    }
...
#[cfg(test)]
#[path = "../../tests/unit/reverse_search/worker.rs"]
mod tests;
