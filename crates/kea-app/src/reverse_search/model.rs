    ...
    pub fn search(
        ...
        for entry in &self.entries {
            if entry
                .scope
                .as_deref()
                .is_some_and(|scope| !same_directory(Some(scope), directory))
            {
                continue;
            }
            ...
            let Some(quality) = words.iter().try_fold(0_i64, |sum, word| {
                fuzzy_score(&body, word)
                    .max(fuzzy_score(&name, word))
                    .map(|score| sum + score)
            }) else {
                continue;
            };
            ...
            hits.push(Hit {
                entry: entry.clone(),
                occurrence_ids: if entry.name.is_none() {
                    vec![entry.id.clone()]
                } else {
                    vec![]
                },
...
#[cfg(test)]
#[path = "../../tests/unit/reverse_search/model.rs"]
mod tests;
