...
impl Pty {
    pub fn spawn(command: &[OsString], size: Size) -> Result<Self> {
        ...
        let child = pair
            ...
            .context("spawning terminal program")?;
        drop(pair.slave);
        ...
        thread::spawn(move || {
            ...
            loop {
                match reader.read(&mut buffer) {
                    ...
                    Ok(n) => {
                        if output_tx
                            .send(Message::Output(buffer[..n].to_vec()))
                            .is_err()
                        {
                            return;
                        }
                    ...
                    Err(e) => {
                        let _ = output_tx.send(Message::Error(e.to_string()));
                        break;
                    }
                }
            }
    ...
    pub fn try_recv(&mut self) -> Option<Message> {
        ...
        if matches!(message, Message::Eof) {
            self.eof = true;
        }
        Some(message)
    }
    pub fn send(&self, bytes: Vec<u8>) -> Result<()> {
        if bytes.len() > kea_core::MAX_OUTPUT {
            anyhow::bail!("input exceeds 1 MiB");
        }
        self.input
            .as_ref()
            .context("PTY input is closed")?
            .try_send(bytes)
            .context("PTY input queue is full or closed")
    }
    pub fn resize(&self, size: Size) -> Result<()> {
        Size::new(size.columns, size.rows)?;
        self.master
            .as_ref()
            .context("PTY is closing")?
